...
data modify storage ca:ram stack[-1].structure.arrangement append value {type: "AIR"}

execute if block ~ ~ ~ #ca:ml/arc_glm/block run \
    data modify storage ca:ram stack[-1].structure.arrangement[-1] set value { type: "block",  pos: [ 0.0f , 0.0f, 0.0f ] }

execute if block ~ ~ ~ #ca:ml/arc_glm/block run \
    scoreboard players add #$.sneak_peek.block ca.ml.arc_glm.fun 1


execute if block ~ ~ ~ #minecraft:stairs run \
    function ca:ml/arc_glm/structure/private/serialise/block_type/stairs

execute if block ~ ~ ~ #minecraft:slabs run \
    function ca:ml/arc_glm/structure/private/serialise/block_type/slab

execute if block ~ ~ ~ #minecraft:walls run \
    function ca:ml/arc_glm/structure/private/serialise/block_type/wall
...
execute store result storage ca:ram stack[-1].structure.arrangement[-1].pos[0] float 0.001 run \
    scoreboard players get #$.translation.x ca.ml.arc_glm.fun

execute store result storage ca:ram stack[-1].structure.arrangement[-1].pos[1] float 0.001 run \
    scoreboard players get #$.translation.y ca.ml.arc_glm.fun

execute store result storage ca:ram stack[-1].structure.arrangement[-1].pos[2] float 0.001 run \
