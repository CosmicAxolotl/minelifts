...
execute \
    ...
    if data storage ca:ram stack[-1].greater_component_is.x run \
        tag @s add dvg_abl.facing.east
execute \
    if score #$.tmp.vec.magnitude ca.dvg_abl.fun matches 0.. \
    if data storage ca:ram stack[-1].greater_component_is.x run \
        tag @s add dvg_abl.facing.west

execute \
    if score #$.tmp.vec.magnitude ca.dvg_abl.fun matches ..-1 \
    if data storage ca:ram stack[-1].greater_component_is.z run \
        tag @s add dvg_abl.facing.north
execute \
    if score #$.tmp.vec.magnitude ca.dvg_abl.fun matches 0.. \
    if data storage ca:ram stack[-1].greater_component_is.z run \
        tag @s add dvg_abl.facing.south

execute \
    if score #$.tmp.vec.magnitude ca.dvg_abl.fun matches ..-1 \
    if data storage ca:ram stack[-1].greater_component_is.y run \
        tag @s add dvg_abl.facing.down
execute \
    if score #$.tmp.vec.magnitude ca.dvg_abl.fun matches 0.. \
    if data storage ca:ram stack[-1].greater_component_is.y run \
        tag @s add dvg_abl.facing.up
